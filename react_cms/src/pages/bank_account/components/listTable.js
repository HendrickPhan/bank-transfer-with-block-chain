...
export default function ListTable(props) {
  ...
  const handleAddBalanceClick = (accountNumber) => {
    props.setOpenAddBalanceModal(true);
  ...
  const fetchData = useCallback(() => {
    fetch(Settings.API_URL + `bank-account/${props.userId}?page=${page + 1}&limit=${rowsPerPage}`)
      .then(res => res.json())
      .then(
        (result) => {
          setIsLoaded(true);
              ...
              {data.data.map((row, rowIdx) => (
                <TableRow key={rowIdx}>
                  ...
                  >
                    ...
                    <Button
                      variant="contained"
                      onClick={() => handleAddBalanceClick(row['account_number'])}
                      color='default' 
                      className={classes.button}
                    >
                      Add Balance
